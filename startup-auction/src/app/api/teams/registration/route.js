...
export async function POST(request) {
  try {
    ...
    // Validate team size: minimum 1 (captain only), maximum 3 (captain + 2 members)
    const membersArray = Array.isArray(team_members) ? team_members : [];
    if (membersArray.length > 2) {
      return NextResponse.json(
        { error: "Maximum 2 team members allowed (plus captain). Total team size cannot exceed 3." },
        { status: 400 }
      );
    }
